    ...
    if ($request_type === 'predict') {
        ...
        foreach ($boundariesToConsider as $targetClassKey => $boundary) {
            ...
            if ($totalValid > 0) {
                ...
                $startMiddle = max(0, floor($totalValid / 2) - floor($numPathsToShow / 3 / 2));
                for ($i = $startMiddle; $i < min($totalValid, floor($totalValid / 2) + ceil($numPathsToShow / 3 / 2)); $i++) {
                    if (isset($allCombinations[$i]) && !in_array($allCombinations[$i], $paths)) {
                        $paths[] = $allCombinations[$i];
                    }
                }
                // Add the bottom few
